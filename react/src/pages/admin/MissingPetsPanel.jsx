...
function StatusBadge({ status }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wide border ${s.bg} ${s.text} ${s.border}`}>
...
function TypeBadge({ type }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wide border ${isLost ? "bg-red-100 text-red-700 border-red-300" : "bg-green-100 text-green-800 border-green-300"}`}>
      ...
      <div className="bg-white rounded-2xl max-w-lg w-full max-h-[92vh] overflow-hidden flex flex-col shadow-2xl border border-black/10"
        ...
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-black/[0.08] flex-shrink-0 gap-2">
          ...
          <div className="flex items-center gap-1.5 flex-shrink-0 flex-wrap justify-end">
            ...
            <StatusBadge status={pet.status ?? "pending"} />
            {pet.resolvedByUser && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wide border bg-blue-100 text-blue-700 border-blue-300">
...
/* ─────────────────────────────────────────────
   MOBILE PET CARD  (replaces table row on small screens)
───────────────────────────────────────────── */
function PetCard({ pet, onView, onEdit, onApprove, onReject, onDelete }) {
  const [imgErr, setImgErr] = useState(false);
  const isCat = pet.species?.toLowerCase() === "cat";


  return (
    <div className="border-b border-[rgba(180,140,60,0.11)] last:border-b-0">
      {/* Main tappable area */}
      <div className="flex gap-3 p-4 cursor-pointer hover:bg-[rgba(90,170,48,0.04)] active:bg-[rgba(90,170,48,0.08)] transition-colors"
        onClick={onView}>


        {/* Thumbnail */}
        <div className="w-16 h-16 rounded-2xl overflow-hidden bg-[rgba(180,140,60,0.12)] flex items-center justify-center flex-shrink-0 relative">
          {!imgErr ? (
            <img src={getPetPhotoUrl(pet)} alt="" className="w-full h-full object-cover"
              onError={() => setImgErr(true)} />
          ) : (
            <span className="flex items-center justify-center w-full h-full">
              {isCat
                ? <Icons.Cat size={32} style={{ color: "#9aaa80" }} />
                : <Icons.Dog size={32} style={{ color: "#9aaa80" }} />}
            </span>
          )}
          {pet.resolvedByUser && (
            <div className="absolute inset-0 bg-blue-600/60 flex items-center justify-center rounded-2xl">
              <Icons.Home size={18} style={{ color: "#fff" }} />
            </div>
          )}
        </div>
        ...
        <div className="flex-1 min-w-0 flex flex-col justify-center gap-1">
          ...
          <div className="flex items-center justify-between gap-2">
            <p className="font-black text-[15px] text-[#1a4a08] truncate leading-tight m-0">
              {pet.name || "Unknown"}
            </p>
            <TypeBadge type={pet.type} />
          </div>
          ...
          {/* Species · Breed */}
          <p className="text-[12px] font-semibold text-[#6a7a50] truncate m-0">
          ...
          <div className="flex items-center gap-3 mt-0.5">
            {pet.area && (
              <span className="text-[11px] font-bold text-[#9aaa80] flex items-center gap-1">
                <Icons.MapPin size={10} style={{ color: "#9aaa80" }} /> {pet.area}
              </span>
            )}
            {pet.reportedDate && (
              <span className="text-[11px] font-bold text-[#b4a870] flex items-center gap-1">
                <Icons.Calendar size={10} style={{ color: "#b4a870" }} /> {formatDate(pet.reportedDate)}
              </span>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center flex-shrink-0 self-center">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor"
            strokeWidth="2.5" strokeLinecap="round" className="text-[#c4b880] opacity-60">
      ...
      {/* Action strip */}
      <div className="flex border-t border-[rgba(180,140,60,0.08)]" onClick={e => e.stopPropagation()}>
        {[
          {
            key: "edit",
            Icon: Icons.Edit,
          ...
          .filter(a => a.show)
          .map(({ key, Icon, label, cls, onClick }) => (
            <button key={key} onClick={onClick}
              className={`flex-1 flex flex-col items-center justify-center gap-1 py-2.5 text-[10px] font-black uppercase tracking-wide cursor-pointer bg-transparent border-none transition-colors ${cls}`}>
              <Icon size={14} />
              {label}
            </button>
          ))}
...
export default function MissingPetsPanel({ show, onStatsChange }) {
  ...
  /*
    Desktop table columns — fixed widths, no greedy 1fr:
    Photo | Type  | Pet/Breed | Species | Area    | Status  | Date    | Actions
    44px  | 72px  | 160px     | 80px    | minmax(120px,1fr) | 110px | 90px | 130px
  */
  // Fixed columns — nothing grows unbounded. Total ~860px which fits most admin panels.
  // Photo | Type | Pet/Breed | Species | Area | Status | Date | Actions
  const GRID = "44px 1fr 2fr 1fr 1.5fr 1.2fr 1fr auto";
  ...
  return (
    <div className="font-[Nunito,sans-serif]">
      ...
      <div className="flex gap-2 flex-wrap items-center mb-5">
        <div className="flex-1 relative" style={{ minWidth: 160 }}>
      ...
      {!loading && filtered.length > 0 && (
        <div className="bg-[rgba(255,248,225,0.9)] border border-[rgba(180,140,60,0.28)] rounded-2xl overflow-hidden shadow-sm">
          ...
          {/* ─── DESKTOP TABLE ─── hidden on mobile via responsive class */}
          <div className="hidden md:block overflow-x-auto">
  <table className="w-full border-collapse">
    <thead>
      <tr className="bg-[rgba(180,140,60,0.08)] border-b border-[rgba(180,140,60,0.18)]">
        {["Photo", "Type", "Pet / Breed", "Species", "Area", "Status", "Date", "Actions"].map(h => (
          <th key={h} className="text-left text-[10px] font-black uppercase tracking-widest text-[#6a7a50] px-4 py-2.5 whitespace-nowrap">
            {h}
          </th>
        ))}
      </tr>
    ...
    <tbody>
      {pagedPets.map((pet, i) => {
        ...
        return (
          <tr key={pet.id}
            className="cursor-pointer hover:bg-[rgba(90,170,48,0.05)] transition-colors"
            style={{ borderBottom: isLast ? "none" : "1px solid rgba(180,140,60,0.11)" }}
            onClick={() => setSelected(pet)}>


            {/* Photo */}
            <td className="px-4 py-3">
              <div className="w-10 h-10 rounded-xl overflow-hidden bg-[rgba(180,140,60,0.12)] flex items-center justify-center relative">
                <img src={getPetPhotoUrl(pet)} alt=""
                  className="w-full h-full object-cover"
                  onError={e => { e.target.style.display = "none"; e.target.nextSibling.style.display = "flex"; }} />
                <span className="hidden items-center justify-center w-full h-full absolute inset-0">
                  {isCat ? <Icons.Cat size={22} style={{ color: "#9aaa80" }} /> : <Icons.Dog size={22} style={{ color: "#9aaa80" }} />}
                </span>
                {pet.resolvedByUser && (
                  <div className="absolute inset-0 bg-blue-600/60 flex items-center justify-center rounded-xl">
                    <Icons.Home size={12} style={{ color: "#fff" }} />
                  </div>
                )}
              </div>
            </td>
            ...
            {/* Type */}
            <td className="px-4 py-3"><TypeBadge type={pet.type} /></td>


            {/* Pet / Breed */}
            <td className="px-4 py-3">
              <p className="font-black text-sm text-[#1a4a08] m-0 truncate max-w-[200px]">{pet.name || "Unknown"}</p>
              <p className="text-[11px] font-bold text-[#6a7a50] m-0 truncate max-w-[200px]">{pet.breed || "—"}</p>
            </td>


            {/* Species */}
            <td className="px-4 py-3 text-xs font-bold text-[#3a5020]">{pet.species || "—"}</td>


            {/* Area */}
            <td className="px-4 py-3 text-xs font-bold text-[#3a5020] max-w-[160px]">
              <p className="m-0 truncate">{pet.area || "—"}</p>
            </td>
            ...
            <td className="px-4 py-3">
              {pet.resolvedByUser
                ? <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wide border bg-blue-100 text-blue-700 border-blue-300">
                    <span className="w-1.5 h-1.5 rounded-full bg-blue-400 flex-shrink-0" /> Reunited
                  </span>
                : <StatusBadge status={pet.status ?? "pending"} />
              }
            </td>
            ...
            {/* Date */}
            <td className="px-4 py-3 text-[11px] font-bold text-[#6a7a50] whitespace-nowrap">
            ...
            <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
              <div className="flex gap-1 flex-nowrap">
                {rowActions.map(({ key, Icon, cls, title, onClick }) => (
                  <button key={key} onClick={onClick} title={title}
                    className={`w-7 h-7 rounded-lg border cursor-pointer transition-opacity hover:opacity-70 flex items-center justify-center flex-shrink-0 ${cls}`}>
                    <Icon size={12} />
                  </button>
                ))}
              </div>
          ...
          <div className="md:hidden">
            {pagedPets.map(pet => (
              <PetCard
                key={pet.id}
                pet={pet}
                onView={() => setSelected(pet)}
                onEdit={() => setEditPet(pet)}
                onApprove={() => setConfirmAct({ pet, action: "approve" })}
                onReject={() => setConfirmAct({ pet, action: "reject" })}
                onDelete={() => setConfirmDel(pet)}
              />
            ))}
          </div>
        </div>
