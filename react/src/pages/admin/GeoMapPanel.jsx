// ── GEO MAP PANEL — GPS-driven navigation with real device movement ───────────
...
// ── Total route distance in km from a given index to end ─────────────────────
function remainingDistanceKm(points, fromIdx) {
  let d = 0;
  for (let i = fromIdx; i < points.length - 1; i++) {
    d += haversineMetres(points[i], points[i+1]);
  }
  return d / 1000;
}
...
// ── Snap GPS coord to nearest route point index ───────────────────────────────
...
// ── Derive next-turn instruction from ORS steps ───────────────────────────────
// steps: array of ORS route step objects { instruction, distance, duration }
// We track which step the vehicle is in based on cumulative distance
...
// ── Turn icon mapping (ORS type codes) ───────────────────────────────────────
  ...
  return L.divIcon({
    ...
    ">
      <svg viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg">
        <ellipse cx="20" cy="25" rx="11" ry="8" fill="#1c4f09" stroke="#fff" stroke-width="1.5"/>
        <ellipse cx="20" cy="18" rx="7" ry="6" fill="#2a7010" stroke="#fff" stroke-width="1"/>
        <ellipse cx="20" cy="16" rx="4.5" ry="3" fill="rgba(180,230,255,0.9)" stroke="#5aaa30" stroke-width="0.8"/>
        <ellipse cx="14" cy="22" rx="2" ry="1.4" fill="#fffbe0" opacity="0.9"/>
        <ellipse cx="26" cy="22" rx="2" ry="1.4" fill="#fffbe0" opacity="0.9"/>
        <polygon points="20,3 23,10 20,8 17,10" fill="#5aaa30" stroke="#fff" stroke-width="0.8"/>
        <ellipse cx="12" cy="28" rx="2.8" ry="2.2" fill="#111" stroke="#888" stroke-width="0.7"/>
        ...
        <!-- GPS pulse ring -->
        <circle cx="20" cy="20" r="18" fill="none" stroke="rgba(90,170,48,0.35)" stroke-width="1.5" stroke-dasharray="4 3"/>
      </svg>
    </div>`,
    iconSize: [40, 40],
    iconAnchor: [20, 20],
...
const startIcon = L.divIcon({
  className:"",
  html:`<div style="width:28px;height:28px;border-radius:50%;background:#1c4f09;border:3px solid #fff;box-shadow:0 2px 10px rgba(0,0,0,.5);display:flex;align-items:center;justify-content:center;color:#fff;font-size:12px;font-weight:900">A</div>`,
  iconSize:[28,28],iconAnchor:[14,14],
});
const endIcon = L.divIcon({
  className:"",
  html:`<div style="width:28px;height:28px;border-radius:50%;background:#c03030;border:3px solid #fff;box-shadow:0 2px 10px rgba(0,0,0,.5);display:flex;align-items:center;justify-content:center;color:#fff;font-size:12px;font-weight:900">B</div>`,
  iconSize:[28,28],iconAnchor:[14,14],
});
...
function MapController({flyTarget, routePoints, vehiclePos, isNavActive}) {
  ...
  useEffect(()=>{
    if (routePoints?.length>1 && !isNavActive) {
      map.fitBounds(L.latLngBounds(routePoints),{padding:[40,40],animate:true,duration:1.0});
    }
  },[routePoints, isNavActive]);
  // During navigation, keep vehicle centred with a slight forward offset
  useEffect(()=>{
    if (vehiclePos && isNavActive) {
      map.panTo([vehiclePos.lat, vehiclePos.lng],{animate:true,duration:0.6,easeLinearity:0.5});
    }
  },[vehiclePos, isNavActive]);
  return null;
}
...
function AccuracyCircle({pos, accuracy}) {
  ...
  return (
    <CircleMarker
      ...
      radius={Math.min(accuracy / 2, 60)}
      pathOptions={{color:"#5aaa30",fillColor:"#5aaa30",fillOpacity:0.08,weight:1,dashArray:"4 3"}}
    />
  );
}
...
function DirectionsPanel({destination, onRouteReady, onClose}) {
  ...
  const getRoute = async () => {
    ...
    try {
      let startCoord;
      if (useGeo && geoCoords) {
        startCoord = {...geoCoords, label:"My Location"};
      } else {
        startCoord = await geocodeLocation(fromText);
      }
      const res = await fetch("/ors/v2/directions/driving-car/geojson", {
        method:"POST",
        headers:{"Content-Type":"application/json","Authorization":ORS_KEY},
        body:JSON.stringify({
          coordinates:[[startCoord.lng,startCoord.lat],[destination.lng,destination.lat]],
          instructions: true,
      ...
      onRouteReady({
        ...
        startLabel: startCoord.label || fromText,
        steps,
        totalMetres: props.distance,
      });
    } catch(e) {
    ...
    <div style={{background:"rgba(42,112,16,.05)",border:"1px solid rgba(42,112,16,.2)",borderRadius:14,padding:"1rem 1.1rem"}}>
      ...
      <div style={{marginBottom:8}}>
        ...
        <div style={{display:"flex",gap:6}}>
          <input value={fromText} onChange={e=>{setFromText(e.target.value);setUseGeo(false);setError("");}}
            onKeyDown={e=>e.key==="Enter"&&fromText.trim()&&getRoute()}
            placeholder="Type any city, address, barangay…"
          ...
          <button onClick={getLocation} title="Use GPS location"
            style={{padding:"8px 12px",borderRadius:8,border:"1px solid #c8b878",background:useGeo?"#1c4f09":"rgba(255,250,232,.8)",color:useGeo?"#fff":"#5a7040",fontSize:14,cursor:"pointer",fontWeight:700,whiteSpace:"nowrap"}}>
            📍 GPS
          </button>
        </div>
        <div style={{fontSize:10,color:"#9aaa80",marginTop:3,fontWeight:600}}>Examples: Laoag, Vigan City, Baguio, Manila…</div>
      </div>
      ...
      {error&&(
        <div style={{fontSize:12,color:"#c03030",fontWeight:600,marginBottom:8,padding:"6px 8px",background:"rgba(192,48,48,.07)",borderRadius:6}}>⚠ {error}</div>
      )}
...
function NavHUD({ route, progress, gpsSpeed, gpsAccuracy, nextTurn, onStop }) {
  ...
  return (
    <div style={{
      borderRadius:16,
      overflow:"hidden",
      boxShadow:"0 6px 24px rgba(28,79,9,.35)",
      border:"1.5px solid rgba(90,170,48,.3)",
    }}>
      {/* ── Next turn banner ── */}
      {nextTurn && !arrived && (
        <div style={{
          background:"linear-gradient(135deg,#1c4f09,#1a5208)",
          padding:"10px 16px",
          display:"flex",
          alignItems:"center",
          gap:12,
          ...
          <div style={{
            ...
            {turnIcon(nextTurn.instruction)}
          </div>
          <div style={{flex:1,minWidth:0}}>
            <div style={{color:"rgba(255,255,255,.65)",fontSize:10,fontWeight:800,textTransform:"uppercase",letterSpacing:".1em",marginBottom:2}}>
              {nextTurn.distToTurn && `In ${nextTurn.distToTurn}`}
            </div>
            <div style={{color:"#fff",fontSize:14,fontWeight:800,lineHeight:1.3,
          ...
          <div style={{flexShrink:0,textAlign:"center"}}>
            <div style={{
              ...
              color: gpsAccuracy == null ? "#c87820"
                   : gpsAccuracy < 15   ? "#5aaa30"
                   : gpsAccuracy < 40   ? "#c87820"
                   : "#c03030",
            }}>
              {gpsAccuracy == null ? "⚡ GPS" : gpsAccuracy < 15 ? "📡 High" : gpsAccuracy < 40 ? "📡 Mid" : "📡 Low"}
            </div>
            {gpsAccuracy != null && (
              <div style={{color:"rgba(255,255,255,.5)",fontSize:9,fontWeight:600}}>±{Math.round(gpsAccuracy)}m</div>
            )}
          </div>
        </div>
      )}
      ...
      {/* ── Arrived banner ── */}
      {arrived && (
        <div style={{background:"linear-gradient(135deg,#1a5208,#2a7010)",padding:"12px 16px",display:"flex",alignItems:"center",gap:12}}>
          <div style={{fontSize:28}}>🏁</div>
          <div style={{color:"#fff",fontSize:15,fontWeight:900}}>You have arrived at your destination!</div>
        </div>
      )}

      {/* ── Stats row ── */}
      <div style={{
        background:"rgba(28,79,9,.95)",
        padding:"10px 16px",
        display:"flex",
        alignItems:"center",
        gap:0,
        ...
        <div style={{flex:1,borderRight:"1px solid rgba(255,255,255,.1)",paddingRight:12,textAlign:"center"}}>
          <div style={{color:"#5aaa30",fontSize:20,fontWeight:900,lineHeight:1}}>
            {distRem < 1 ? `${Math.round(distRem*1000)} m` : `${distRem.toFixed(1)} km`}
          </div>
        ...
        <div style={{flex:1,paddingLeft:12,display:"flex",flexDirection:"column",alignItems:"center",gap:6}}>
          ...
          <div style={{position:"relative",width:36,height:36}}>
            <svg viewBox="0 0 36 36" style={{transform:"rotate(-90deg)"}}>
              ...
              <circle cx="18" cy="18" r="15" fill="none" stroke="#5aaa30" strokeWidth="3"
                strokeDasharray={`${(pct/100)*94.2} 94.2`} strokeLinecap="round"/>
            </svg>
            <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",
              color:"#fff",fontSize:9,fontWeight:900}}>{pct}%</div>
          </div>
          <button onClick={onStop}
            style={{padding:"3px 10px",borderRadius:8,border:"1px solid rgba(255,100,100,.5)",
              background:"rgba(192,48,48,.3)",color:"#ffaaaa",fontSize:10,fontWeight:800,cursor:"pointer",whiteSpace:"nowrap"}}>
            ✕ End Nav
          </button>
        </div>
      </div>
...
// ── Route info bar (before navigation starts) ─────────────────────────────────
    ...
    <div style={{background:"linear-gradient(135deg,rgba(28,79,9,.08),rgba(42,112,16,.05))",border:"1.5px solid rgba(42,112,16,.25)",borderRadius:14,padding:"1rem 1.2rem",display:"flex",alignItems:"center",gap:14,flexWrap:"wrap"}}>
      ...
      <div style={{display:"flex",gap:12,alignItems:"center",flexShrink:0}}>
        <div style={{textAlign:"center"}}>
          <div style={{fontSize:18,fontWeight:900,color:"#1c4f09",lineHeight:1}}>{route.distance}<span style={{fontSize:10}}> km</span></div>
          <div style={{fontSize:9,color:"#9aaa80",fontWeight:700,textTransform:"uppercase"}}>Distance</div>
        </div>
        <div style={{textAlign:"center"}}>
          <div style={{fontSize:18,fontWeight:900,color:"#c87820",lineHeight:1}}>{route.duration}<span style={{fontSize:10}}> min</span></div>
          <div style={{fontSize:9,color:"#9aaa80",fontWeight:700,textTransform:"uppercase"}}>Drive Time</div>
        </div>
        <div style={{textAlign:"center"}}>
          <div style={{fontSize:12,fontWeight:800,color:"#5aaa30",lineHeight:1}}>{route.steps?.length || 0}</div>
          <div style={{fontSize:9,color:"#9aaa80",fontWeight:700,textTransform:"uppercase"}}>Turns</div>
        </div>
        <button onClick={onStartNav}
          style={{padding:"9px 18px",borderRadius:10,border:"none",background:"linear-gradient(135deg,#1c4f09,#2a7010)",color:"#fff",fontSize:13,fontWeight:800,cursor:"pointer",whiteSpace:"nowrap",
            boxShadow:"0 3px 12px rgba(28,79,9,.35)"}}>
          🚗 Start Navigation
        </button>
        <button onClick={onClear}
          style={{padding:"8px 12px",borderRadius:8,border:"1px solid rgba(192,48,48,.3)",background:"rgba(192,48,48,.07)",color:"#c03030",fontSize:12,fontWeight:800,cursor:"pointer"}}>
          ✕ Clear
        </button>
      </div>
    </div>
            ...
            {[{label:"Approved",count:sc.Approved,color:"#5aaa30"},{label:"Pending",count:sc.Pending,color:"#c87820"},{label:"Rejected",count:sc.Rejected,color:"#c03030"}].map(s=>{
              ...
              return (
                <div key={s.label} style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                  ...
                  <div style={{flex:1,height:7,borderRadius:4,background:"#e8dfc0",overflow:"hidden"}}><div style={{height:"100%",width:`${Math.round(s.count/t*100)}%`,background:s.color,borderRadius:4}}/></div>
...
export default function GeoMapPanel({show}) {
  ...
  // ── Stop navigation — clears GPS watch ──────────────────────────────────────
  ...
  const startNav = useCallback(()=>{
    ...
    setNavProgress(0);
    setTravelledMetres(0);
    setVehiclePos({lat:route.points[0][0], lng:route.points[0][1]});
    ...
    watchIdRef.current = navigator.geolocation.watchPosition(
      (pos) => {
        ...
        // Compute travelled distance (sum of haversine up to snapped index)
        let dist = 0;
        for (let i = 0; i < idx; i++) {
          dist += haversineMetres(r.points[i], r.points[i+1]);
        }

        setVehiclePos({ lat: r.points[idx][0], lng: r.points[idx][1] });
        setVehicleRotation(rot);
        setNavProgress(idx);
        setTravelledMetres(dist);

        // Speed: Geolocation API gives m/s, convert to km/h
        ...
        // Auto-stop when within 30m of destination
        const destPt = r.points[r.points.length - 1];
        const distToDest = haversineMetres([lat, lng], destPt);
        if (distToDest < 30) {
          setNavProgress(r.points.length - 1);
          setVehiclePos({ lat: destPt[0], lng: destPt[1] });
          // Don't call stopNav immediately so user sees "arrived" state
          setTimeout(stopNav, 4000);
        }
      },
      (err) => {
        console.warn("GPS error:", err.message);
        // Show error in HUD but keep nav active so user can manually end
        setGpsSpeed(null);
        setGpsAccuracy(null);
      },
      {
  ...
  // Derived: next turn instruction
  const nextTurn = navActive && route?.steps
    ? getNextTurn(route.steps, travelledMetres)
    : null;
  ...
  const handleRouteReady = (rd) => {
    setRoute({
      ...rd,
      endLabel: directionsUser
        ? `${directionsUser.first_name} ${directionsUser.last_name} (${directionsUser.city||""})`
        : rd.end.label || "",
    });
    setDirectionsUser(null);
  };
  const clearRoute = () => {
    setRoute(null);
    setDirectionsUser(null);
  ...
  const provStats = Object.keys(PROVINCES).map(p=>({
    name:p, color:PROVINCES[p].color,
    count:geocoded.filter(u=>u._geo.province===p).length,
    active:geocoded.filter(u=>u._geo.province===p&&u.is_active).length,
  }));
    ...
    <div className="flex flex-col gap-5">
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            {["all","active","inactive"].map(f=>(
              <button key={f} onClick={()=>setFilter(f)}
                style={{padding:"6px 14px",borderRadius:20,fontSize:12,fontWeight:800,border:"1.5px solid",cursor:"pointer",transition:"all .15s",
                  background:filter===f?"#1c4f09":"transparent",borderColor:filter===f?"#1c4f09":"#ddd0a8",color:filter===f?"#fff":"#7a9060"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12}}>
        {provStats.map(p=>(
          <div key={p.name} onClick={()=>setProvFilter(provFilter===p.name?"all":p.name)}
            style={{borderRadius:14,padding:"1rem 1.2rem",cursor:"pointer",transition:"all .15s",
              background:provFilter===p.name?`${p.color}18`:"#fffce8",
              border:`1.5px solid ${provFilter===p.name?p.color:"rgba(200,180,100,.3)"}`,
              boxShadow:provFilter===p.name?`0 4px 16px ${p.color}28`:"none",
              transform:provFilter===p.name?"translateY(-2px)":"none"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
        {[
          ...
          {val:users.length-geocoded.length, label:"Outside Region", icon:"↗",color:"#c87820"},
        ].map(s=>(
      ...
      {/* GPS Nav HUD — shown during active navigation */}
      {navActive && route && (
        <NavHUD
          route={route}
      ...
      {/* Route info bar — shown after route calculated, before nav starts */}
      {!navActive && route && (
        <RouteInfoBar route={route} onStartNav={startNav} onClear={clearRoute}/>
      )}

      {/* Map + sidebar */}
      <div style={{display:"grid",gridTemplateColumns:"1fr 300px",gap:14}}>
        <div style={{borderRadius:20,overflow:"hidden",
          border:`1.5px solid ${navActive?"rgba(28,79,9,.6)":"rgba(90,160,48,.35)"}`,
          boxShadow:navActive?"0 4px 28px rgba(28,79,9,.3)":"0 4px 20px rgba(30,60,10,.08)",
          transition:"all .3s"}}>
          ...
          <div style={{padding:"10px 14px",background:navActive?"rgba(28,79,9,.97)":"rgba(255,248,220,.97)",borderBottom:"1px solid #e8dfc0",transition:"background .3s"}}>
            {directionsUser && !route ? (
              <DirectionsPanel
                destination={{lat:directionsUser._geo.lat,lng:directionsUser._geo.lng,
                  label:`${directionsUser.first_name} ${directionsUser.last_name}, ${directionsUser.city||""}`}}
              ...
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                {navActive ? (
                  <div style={{display:"flex",alignItems:"center",gap:8,flex:1}}>
                    <div style={{width:8,height:8,borderRadius:"50%",background:"#5aaa30",
                      animation:"gpsPulse 1.2s ease-in-out infinite"}}/>
                    <span style={{color:"#5aaa30",fontSize:12,fontWeight:800,flex:1}}>
                      🛰 GPS Navigation Active — map follows your position
                    </span>
                    {gpsAccuracy != null && (
                      <span style={{color:"rgba(255,255,255,.5)",fontSize:11,fontWeight:600}}>±{gpsAccuracy}m</span>
                    )}
                  </div>
                ) : (
                  <>
              ...
              {show && (
                <MapContainer center={ILOCOS_CENTER} zoom={ILOCOS_ZOOM} style={{height:"100%",width:"100%"}} zoomControl>
                  ...
                  {Object.entries(PROVINCES).map(([prov,cfg])=>(
                    <Polygon key={prov} positions={cfg.bounds}
                      pathOptions={{color:cfg.color,weight:2,fillColor:cfg.color,
                        fillOpacity:provFilter===prov?.14:.06,dashArray:"5 4"}}>
                  ...
                  {route?.points && (
                    <>
                      ...
                      <Polyline positions={route.points}
                        pathOptions={{color:"rgba(0,0,0,.15)",weight:10,opacity:1}}/>
                      {/* Travelled portion — solid dark green */}
                      {navActive && navProgress > 0 && (
                        <Polyline positions={route.points.slice(0, navProgress + 1)}
                          pathOptions={{color:"#1c4f09",weight:7,opacity:1}}/>
                      )}
                      {/* Remaining / full route — light green dashed */}
                      <Polyline
                        positions={navActive ? route.points.slice(navProgress) : route.points}
                        pathOptions={{color:"#5aaa30",weight:5,opacity:.85,
                          dashArray: navActive ? "10 7" : undefined}}/>
                      {/* A/B markers */}
                  ...
                  {/* GPS accuracy circle */}
                  {navActive && vehiclePos && gpsAccuracy != null && (
                    <AccuracyCircle pos={vehiclePos} accuracy={gpsAccuracy}/>
                  )}

                  {/* Moving vehicle */}
                  {navActive && vehiclePos && (
                    <VehicleMarker pos={vehiclePos} rotation={vehicleRotation}/>
                  )}

                  {/* User pins — hidden during nav to reduce clutter */}
                  {!navActive && filtered.map(u=>{
                    const pc = PROVINCES[u._geo.province]?.color||"#2a7010";
                    return (
                      <CircleMarker key={u.id} center={[u._geo.lat,u._geo.lng]}
                        radius={u.is_active?10:7}
                        pathOptions={{fillColor:u.is_active?pc:"#aaa",color:"#fff",
                          weight:2.5,fillOpacity:u.is_active?.92:.55}}
                          ...
                          <div style={{minWidth:170,fontFamily:"inherit",padding:"2px 0"}}>
                            ...
                            <div style={{display:"flex",gap:6}}>
                              <button onClick={()=>setSelected(u)}
                                style={{flex:1,padding:"5px 8px",borderRadius:7,border:"1px solid rgba(42,112,16,.3)",background:"rgba(42,112,16,.08)",color:"#1c4f09",fontSize:11,fontWeight:800,cursor:"pointer"}}>Details</button>
                              <button onClick={()=>handleDirections(u)}
                                style={{flex:1,padding:"5px 8px",borderRadius:7,border:"none",background:"#1c4f09",color:"#fff",fontSize:11,fontWeight:800,cursor:"pointer"}}>Directions</button>
                            </div>
                          </div>
                        </Popup>
                      </CircleMarker>
          ...
          <div style={{padding:"10px 14px",background:"rgba(255,252,235,.97)",borderTop:"1px solid #e8dfc0",display:"flex",alignItems:"center",gap:16,flexWrap:"wrap"}}>
            ...
            <span style={{fontSize:11,color:"#9aaa80",marginLeft:"auto"}}>
              {navActive ? "🛰 Following GPS position" : "🖱 Click pin → Details or Directions"}
            </span>
          </div>
        </div>
        ...
        <div style={{borderRadius:20,border:"1.5px solid #ddd0a8",background:"#fffce8",overflow:"hidden",display:"flex",flexDirection:"column"}}>
          <div style={{padding:"12px 14px 10px",borderBottom:"1px solid #e8dfc0",background:"rgba(255,248,220,.97)"}}>
            <div style={{fontSize:11,fontWeight:800,color:"#1a4a08",textTransform:"uppercase",letterSpacing:".08em"}}>
              {navActive ? "🚗 Navigation Active" : `Users in Region (${filtered.length})`}
            </div>
          </div>

          {navActive ? (
            /* During navigation: show step-by-step turn list */
            <div style={{overflowY:"auto",flex:1,maxHeight:480,padding:"8px 0"}}>
              {route?.steps?.length ? route.steps.map((step,i)=>{
                // Compute cumulative distance to this step
                let cum = 0;
                for (let j = 0; j < i; j++) cum += route.steps[j].distance || 0;
                const passed = travelledMetres >= cum + (step.distance || 0);
                const isCurrent = !passed && travelledMetres >= cum;
                return (
                  <div key={i} style={{padding:"8px 14px",borderBottom:"1px solid rgba(200,176,100,.1)",
                    display:"flex",alignItems:"center",gap:8,
                    background: isCurrent?"rgba(28,79,9,.08)":passed?"rgba(90,170,48,.04)":"transparent",
                    opacity: passed ? 0.45 : 1,
                    transition:"all .3s"}}>
                    <div style={{width:28,height:28,borderRadius:8,flexShrink:0,
                      background:isCurrent?"#1c4f09":passed?"rgba(90,170,48,.15)":"rgba(42,112,16,.08)",
                      display:"flex",alignItems:"center",justifyContent:"center",fontSize:14}}>
                      {passed?"✓":turnIcon(step.instruction)}
                    </div>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontSize:11,fontWeight:800,color:isCurrent?"#1c4f09":passed?"#9aaa80":"#1a2e0a",
                        lineHeight:1.3,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
                        {step.instruction}
                      </div>
                      <div style={{fontSize:10,color:"#9aaa80",fontWeight:600,marginTop:1}}>
                        {step.distance < 1000
                ...
                <div style={{padding:"2rem",textAlign:"center",color:"#9aa880",fontSize:12}}>
                  ...
                  <div key={u.id} style={{padding:"10px 14px",borderBottom:"1px solid rgba(200,176,100,.12)",display:"flex",alignItems:"center",gap:8}}>
                    ...
                    <div style={{display:"flex",gap:4,flexShrink:0}}>
                      <button onClick={()=>setSelected(u)} title="View details"
                        style={{width:26,height:26,borderRadius:6,border:"1px solid rgba(42,112,16,.25)",background:"rgba(42,112,16,.07)",color:"#1c4f09",fontSize:12,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>👁</button>
                      <button onClick={()=>handleDirections(u)} title="Get directions"
                        style={{width:26,height:26,borderRadius:6,border:"none",background:"#1c4f09",color:"#fff",fontSize:12,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>🗺</button>
                    </div>
                  </div>
                );
              })}
